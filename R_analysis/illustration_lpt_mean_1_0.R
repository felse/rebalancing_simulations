...
output_pdf = paste(working_directory, "/pdfs/", "illustration_lpt_mean_1_0", ".pdf", sep = "")
...
tempplot=ggplot(data=df_current_distr,aes_string(x="rebalanced", y="osd_load", fill="moved")) + 
  geom_hline(yintercept = c(average_load), color = "grey") +
  theme(panel.grid.minor=element_blank()) +
  geom_bar(stat="identity", colour="black",)+
  facet_grid( ~ osd_id) +
  scale_fill_manual(values=color_theme) + 
  labs(x = "", y = "") +
  scale_y_continuous(breaks = c(average_load), labels = c("Limit=\nLower\nbound")) +
